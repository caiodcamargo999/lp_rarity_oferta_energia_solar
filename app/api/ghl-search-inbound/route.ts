...
export async function GET() {
  try {
    ...
    // Procurar por qualquer pipeline que contenha "Outbound"
    const outboundPipelines = data.pipelines?.filter((pipeline: any) => 
      pipeline.name?.toLowerCase().includes('outbound')
    ) || []

    // Procurar por qualquer pipeline que contenha "Energia Solar"
    ...
    return NextResponse.json({
      success: true,
      message: 'Busca por pipelines concluída',
      data: {
